...
var _emscripten_create_audio_context = options => {
  ...
  options >>= 2;
  let opts = options ? {
    latencyHint: SAFE_HEAP_LOAD(options * 4, 4, 1) ? UTF8ToString(SAFE_HEAP_LOAD(options * 4, 4, 1)) : void 0,
    sampleRate: SAFE_HEAP_LOAD((options + 1) * 4, 4, 0) || void 0
  } : void 0;
...
var _emscripten_create_wasm_audio_worklet_node = (contextHandle, name, options, callback, userData) => {
  ...
  function readChannelCountArray(heapIndex, numOutputs) {
    let channelCounts = [];
    while (numOutputs--) channelCounts.push(SAFE_HEAP_LOAD(heapIndex++ * 4, 4, 1));
    return channelCounts;
  }
  let opts = options ? {
    numberOfInputs: SAFE_HEAP_LOAD(options * 4, 4, 0),
    numberOfOutputs: SAFE_HEAP_LOAD((options + 1) * 4, 4, 0),
    outputChannelCount: SAFE_HEAP_LOAD((options + 2) * 4, 4, 1) ? readChannelCountArray(SAFE_HEAP_LOAD((options + 2) * 4, 4, 1) >> 2, SAFE_HEAP_LOAD((options + 1) * 4, 4, 0)) : void 0,
    processorOptions: {
      "cb": callback,
      "ud": userData,
...
var _emscripten_create_wasm_audio_worklet_processor_async = (contextHandle, options, callback, userData) => {
  ...
  options >>= 2;
  let audioParams = [], numAudioParams = SAFE_HEAP_LOAD((options + 1) * 4, 4, 1), audioParamDescriptors = SAFE_HEAP_LOAD((options + 2) * 4, 4, 1) >> 2, i = 0;
  while (numAudioParams--) {
    audioParams.push({
      name: i++,
      defaultValue: SAFE_HEAP_LOAD_D(audioParamDescriptors++ * 4, 4, 0),
      minValue: SAFE_HEAP_LOAD_D(audioParamDescriptors++ * 4, 4, 0),
      maxValue: SAFE_HEAP_LOAD_D(audioParamDescriptors++ * 4, 4, 0),
      automationRate: [ "a", "k" ][SAFE_HEAP_LOAD(audioParamDescriptors++ * 4, 4, 1)] + "-rate"
    });
  }
  EmAudio[contextHandle].audioWorklet.bootstrapMessage.port.postMessage({
    ...
    "_wpn": UTF8ToString(SAFE_HEAP_LOAD(options * 4, 4, 1)),
    "ap": audioParams,
    "ch": contextHandle,
    "cb": callback,
    "ud": userData
...
function _emscripten_set_click_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(2, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
...
var _emscripten_start_wasm_audio_worklet_thread_async = (contextHandle, stackLowestAddress, stackSize, callback, userData) => {
  ...
  audioWorklet.addModule("noise_generator.aw.js").then(() => {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, _emscripten_set_click_callback_on_thread, _fd_close, _fd_seek, _fd_write ];
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ emscripten_audio_context_state: _emscripten_audio_context_state,
    ...
    /** @export */ emscripten_resume_audio_context_sync: _emscripten_resume_audio_context_sync,
